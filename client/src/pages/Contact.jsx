  ...
  return (
    <section className='flex flex-col justify-center gap-5 w-full max-w-[500px] mx-auto'>
      <Card className='flex flex-col justify-between px-5 sm:px-12 drop-shadow-2xl'>
        <h1 className='text_gradient_p text-2xl text-center underline-offset-2 pt-2'>Contact Us</h1>
        <form className='flex flex-col w-full h-full gap-3 my-4'>
          <input 
            ...
            className='hover:outline-none placeholder:text-blue-gray-100 p-2 shadow-lg rounded-lg' />
          <input 
            type="text" 
            name="email"
            value={email}
          ...
          <textarea 
            ...
            value={message}
            onChange={handleChange}
            className='max-h-24 hover:outline-none placeholder:text-blue-gray-100 p-2 shadow-lg rounded-lg' 
            cols="10" rows="10" placeholder='Write here..'>
          </textarea>
          <Link 
            to={`mailto:${emailTo}?subject=Regarding ${subject}&body=HeyI'm ${full_name},  ${message}`}
            className='text-center w-full uppercase p-3 rounded-lg my-3 bg-[#1d5e22] text-white opacity-90'>
            send
          </Link>
        </form>
      </Card>
